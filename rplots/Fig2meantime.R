...
fic$colors[fic$title == 'She'] <- 'red'

p <- ggplot(fic, aes(x = date, y = meantime)) + geom_point() + geom_smooth(se = F, color = 'gray50', linetype = 'dashed') +
  theme_bw() + 
...
tiff("/Users/tunder/Dropbox/active/time/images/fig2meantime.tiff", height = 6, width = 9, units = 'in', res=400)
plot(p)
dev.off()
plot(p)
plot(p)
